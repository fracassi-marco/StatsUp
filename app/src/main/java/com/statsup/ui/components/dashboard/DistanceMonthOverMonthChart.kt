...
fun DistanceMonthOverMonthChart(viewModel: DashboardViewModel) {
    ...
    val projectedDist = viewModel.projectedCumulativeDistance()
    val cumDist = viewModel.cumulativeDistance()
    val pastCumDist = viewModel.pastCumulativeDistance()
    val distGoal = viewModel.monthlyDistanceGoal()
    val todayIndex = remember { ZonedDateTime.now().dayOfMonth - 1 }
    val projectedPoints = remember(projectedDist) { projectedDist.map { Point(it.value.toFloat(), "") } }
    val cumulativePoints = remember(cumDist) { cumDist.map { Point(it.value.toFloat(), "") } }
    val pastPoints = remember(pastCumDist) { pastCumDist.map { Point(it.value.toFloat(), "") } }
    val goalPoints = remember(cumDist, distGoal) { cumDist.map { Point(distGoal, "") } }
    LineChart(
        lines = listOf(
            Line(
            ...
            Line(
                ...
                pointDrawer = IndexesPointDrawer(listOf(todayIndex), FilledPointDrawer(color = MaterialTheme.colorScheme.onBackground)),
                startAtZero = true,
